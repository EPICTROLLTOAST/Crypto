...
key = b"YELLOW SUBMARINE" ##already 16 bytes
toEncrypt = PKCS_pad(b"1111111002102764021694451451", 32)
IV = b"0000000000000000" ##16 ascii zeros
blocks = [toEncrypt[x:x+16] for x in range(0, len(toEncrypt), 16)]
...
for x in blocks:
    ...
    print(len(IV))
    xor = b""
    for xb, yb in zip(x, IV):
        _1 = xb ^ yb
        xor += _1.to_bytes(1, 'big')
    print(xor)
    xor = aes.encrypt(xor)
    IV = xor
    encrypted += xor
...
print(encrypted)
aes = AES.new(key, AES.MODE_CBC, IV= b"0000000000000000")
print(aes.decrypt(encrypted))
lyrics = aes.decrypt(base64.b64decode(_in))
print(str(lyrics)[2:-1].replace("\\n","\n"))
